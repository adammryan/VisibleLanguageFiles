// var imageFiles = ["jared-brother","shule","omer", "jared","jared-daughter","akish","morianton",
...
function createTimeline() {
    ...
    // different function for updates?
}
createTimeline();
...
function createTimelineFramework() {
    ...
    .attr('d', 'M 0 ' + parseFloat(timelineOffsetY + tickHeight / 2) + ' L 1200 ' + parseFloat(timelineOffsetY + tickHeight / 2))
    ...
    dates.data(markerNums)
        ...
        .append('text')
            ...
            .attr('y', parseInt(timelineOffsetY + tickHeight + 20))
}

function getTickPath(num) {
    num += 700;
    return "M " + parseInt(num) + " " + timelineOffsetY + " " + parseInt(num) + " " + parseInt(timelineOffsetY + tickHeight);
}

function createTimelineBox() {
    let current = lifespanData.find(obj => obj.name == state.speaker);
    ...
    svg.append('rect')
        ...
        .attr('x', startX)
        .attr('y', timelineOffsetY + (tickHeight - timelineBoxHeight) / 2)
        .attr('width', width)
...
function createSmallIcon(type, speaker) {
    ...
    content.innerHTML = "This is a description of the selected speaker with this speaker. This might be describing family relationships, or word choice relationships."
